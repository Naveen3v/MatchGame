...
class Home extends Component {
  ...
  imageChecked = id => {
    const {displayImg, activeTabId, count} = this.state
    this.setState({
      displayImg: imagesList[Math.ceil(Math.random() * 10)].imageUrl,
      count: count + 1,
    })
  }

  homePage = () => {
    const {activeTabId, count, displayImg} = this.state
    const filterList = imagesList.filter(each => each.category === activeTabId)
    return (
      <>
        <Navbar count={count} />
        <div className="inner">
          <img src={displayImg} alt="match" className="bigimg" />
          ...
          <ul className="imgcont">
            {filterList.map(each => (
              <ThumbNailList
                timgDetails={each}
                key={each.id}
                imageChecked={this.imageChecked}
              />
            ))}
          </ul>
        </div>
  ...
  render() {
    const {display, activeTabId, count} = this.state
    return (
      <div className="container">
        {display ? this.homePage() : this.scorecard()}
      </div>
    )
  }
}
